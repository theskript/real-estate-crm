...
async function assertLeadAccess(sb, user, leadId) {
  const { data: lead } = await scopedTable(sb, user, 'leads').select('assigned_agent_id').eq('id', leadId).maybeSingle();
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    ...
    const access = await assertLeadAccess(sb, user, q.lead_id);
    if (!access.ok) return { statusCode: access.statusCode, headers: CORS, body: JSON.stringify({ error: access.error }) };

    const { data, error } = await scopedTable(sb, user, 'activities')
  ...
  if (event.httpMethod === 'POST') {
    ...
    const access = await assertLeadAccess(sb, user, lead_id);
    if (!access.ok) return { statusCode: access.statusCode, headers: CORS, body: JSON.stringify({ error: access.error }) };

    const { data, error } = await scopedTable(sb, user, 'activities').insert({
    ...
    if (['call', 'sms', 'email', 'showing', 'meeting'].includes(type)) {
      await scopedTable(sb, user, 'leads').update({ last_contacted_at: new Date().toISOString() }).eq('id', lead_id);
    }
    await logAudit({ action: 'Log Activity', username: user.username, role: user.role, details: `${type}: ${(text || '').substring(0, 100)}`, targetId: lead_id, ip, organizationId: user.organization_id });
  ...
  if (event.httpMethod === 'DELETE') {
    ...
    const { data: activity } = await scopedTable(sb, user, 'activities').select('lead_id').eq('id', id).maybeSingle();
    if (!activity) return { statusCode: 404, headers: CORS, body: JSON.stringify({ error: 'Activity not found' }) };
    const access = await assertLeadAccess(sb, user, activity.lead_id);
    if (!access.ok) return { statusCode: access.statusCode, headers: CORS, body: JSON.stringify({ error: access.error }) };
    const { error } = await scopedTable(sb, user, 'activities').delete().eq('id', id);
    if (error) return { statusCode: 500, headers: CORS, body: JSON.stringify({ error: error.message }) };
